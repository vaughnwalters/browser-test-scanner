...
const outputFile = path.resolve( resultsDir, 'browser-tests.wiki' );
...
function main() {
	...
	for ( const file of files ) {
		...
		const tests = [];
		for ( const [ specFile, suites ] of Object.entries( data.tests ) ) {
		...
		groups[ category ].push( {
			...
			specFiles: Object.keys( data.tests ),
			tests
		} );
	}
	...
	console.log( `Wikitext written to ${ outputFile }` );
	console.log( `${ files.length } repos, ${ Object.values( groups ).flat().reduce( ( sum, r ) => sum + r.tests.length, 0 ) } total tests` );
}
